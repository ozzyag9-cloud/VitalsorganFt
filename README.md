...
- `chain/` — standalone chain / Layer 3 protocol simulator for blocks, validator eligibility, incentives, expert councils, ambient proof fusion, and policy guardrails.
- `os/` — Proof of Life OS appliance-image scaffold with node config, systemd services, Docker Compose, and image staging script.
- `hardware/` — Proof Pod multi-sensor and ambient-intelligence reference design.
...
The `chain/` workspace models the future standalone chain or Layer 3. It includes typed life transactions, domain-specific validator quorum, block building, incentive allocation, expert council definitions, ambient sensor fusion, and constitutional guardrails.
...
The first version is a deterministic devnet simulator so protocol economics, policy rules, validator design, and ambient proof-of-life/proof-of-health policies can mature before a production consensus client is selected.

## Proof of Life OS and hardware

The `os/` workspace stages a Proof of Life OS appliance image that can run a node, backend API, oracle adapters, and privacy services. The `hardware/` workspace describes the Proof Pod: a multi-sensor edge node that uses ambient intelligence to generate local proof commitments without exporting raw biometric or environmental data.

```bash
os/scripts/build-os-image.sh
docker compose -f os/docker-compose.yml up
```
...
10. Harden Proof of Life OS with secure boot, measured boot, remote attestation, firewall policy, and reproducible image builds.
11. Prototype Proof Pod hardware with consent controls, secure element signing, and multi-sensor fusion.
12. Commission external smart contract, backend, chain, ZK, OS, hardware, policy, and privacy audits.
