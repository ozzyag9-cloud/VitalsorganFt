...
If the AWS screens are confusing, run this locally:

```bash
npm run vitalos:secrets
```

The wizard prompts for the frontend URL, backend API URL, provider keys, Base Sepolia RPC, oracle address, treasury address, and testnet deployer key. It writes two ignored local files:

- `.env.vitalos.frontend` — values to copy into Amplify frontend environment variables.
- `.env.vitalos.backend` — values to copy into the backend runtime only.

Do not paste real secrets into public chat or commit these generated files.
